...
contract FlightSuretyApp {
    ...
    modifier requireContractOwner() {
        require(
            ...
            "App: Caller is not contract owner"
        );
        _;
    }
    ...
    function registerAirline(
        address _address
    ) external payable requireIsOperational requireIsFullyFundedAirline {
        require(
            !flightSuretyData.isAirlineRegistered(_address),
            "Airline is already a registered."
        );
        require(
            !flightSuretyData.hasAirlineAlreadyVoted(_address, msg.sender),
            "Airline cannot vote twice for same airline"
        );

        uint256 numberOfFundedAirlines = flightSuretyData
            .getNumberOfFundedAirlines();
        uint256 numberOfVotesForAirline = flightSuretyData
            .getNumberOfVotesForAirline(_address) + 1;
        bool _isAirlineRegistered = false;

        if (
            numberOfFundedAirlines < MIN_NUM_AIRLINES_FOR_CONCENSUS ||
            (numberOfFundedAirlines == MIN_NUM_AIRLINES_FOR_CONCENSUS &&
                numberOfVotesForAirline >= numberOfFundedAirlines.div(2))
        ) {
    ...
    ) external requireIsOperational requireIsFullyFundedAirline {
    ...
    function purchaseInsurance(
        ...
        require(
            msg.value <= INSURANCE_COST,
            "The cost of insurance is 1 ether"
        );
    ...
    function getFlightStatus(bytes32 _flightKey) external requireIsOperational {
        ...
        ) = flightSuretyData.getFlightData(_flightKey);
        bytes32 key = keccak256(
            abi.encodePacked(index, airline, flightName, flightDateTime)
        );
