...
contract FlightSuretyData {
    ...
    modifier requireContractOwner() {
        require(
            ...
            "Data: Caller is not contract owner"
        );
        _;
    }
    ...
    function hasAirlineAlreadyVoted(
        address _address,
        address _registeredAirline
    )
        external
        view
    ...
    function _registerAirline(
        ...
        airlines[_address].numberOfVotesReceived = airlines[_address]
    ...
    function isAirlineRegistered(
        address _airline
    ) public view requireIsOperational returns (bool success) {
    ...
    function getNumberOfVotesForAirline(
        address _address
    ...
    function purchaseInsurance(
        ...
        require(
            !passengers[passenger].passengerOnFlight[flightKey],
            "Passenger already insured"
        );
        passengers[passenger].passengerOnFlight[flightKey] = true;
        flights[flightKey].insuredPassengers.push(passenger);
        passengers[passenger].insuranceAmountForFlight[flightKey] =
            insuranceAmount.div(2) +
            insuranceAmount;
        _insuranceBalance = _insuranceBalance.add(
            passengers[passenger].insuranceAmountForFlight[flightKey]
        );
    ...
    function processFlightStatus(
        ...
        if (wasLateAirline) {
            for (
                passenger = 0;
                passenger < flights[flightKey].insuredPassengers.length;
                passenger++
            ) {
                if (
                    passengers[flights[flightKey].insuredPassengers[passenger]]
                        .passengerOnFlight[flightKey]
                ) {
                    payableAmount = passengers[
                        flights[flightKey].insuredPassengers[passenger]
                    ].insuranceAmountForFlight[flightKey];
                    passengers[flights[flightKey].insuredPassengers[passenger]]
                        .insuranceAmountForFlight[flightKey] = 0;
                    passengers[flights[flightKey].insuredPassengers[passenger]]
                        .balance = passengers[
                        flights[flightKey].insuredPassengers[passenger]
                    ].balance.add(payableAmount);
